...
void _rte_srand(uint64_t seedval) { rte_srand(seedval); }

uint64_t _rte_rand(void) { return rte_rand(); }
...
int _rte_bitmap_free(struct rte_bitmap *bmp) { return rte_bitmap_free(bmp); }

void _rte_bitmap_reset(struct rte_bitmap *bmp) { rte_bitmap_reset(bmp); }
...
void _rte_spinlock_init(rte_spinlock_t *sl) { rte_spinlock_init(sl); }

void _rte_spinlock_lock(rte_spinlock_t *sl) { rte_spinlock_lock(sl); }

void _rte_spinlock_unlock(rte_spinlock_t *sl) { rte_spinlock_unlock(sl); }

int _rte_spinlock_trylock(rte_spinlock_t *sl) {
  return rte_spinlock_trylock(sl);
}

int _rte_tm_supported(void) { return rte_tm_supported(); }

void _rte_spinlock_lock_tm(rte_spinlock_t *sl) { rte_spinlock_lock_tm(sl); }

int _rte_spinlock_trylock_tm(rte_spinlock_t *sl) {
  return rte_spinlock_trylock_tm(sl);
}

void _rte_spinlock_unlock_tm(rte_spinlock_t *sl) { rte_spinlock_unlock_tm(sl); }
...
unsigned _rte_lcore_id(void) { return rte_lcore_id(); }

int _rte_errno(void) { return rte_errno; }

uint64_t _rte_get_tsc_cycles(void) { return rte_get_tsc_cycles(); }

uint64_t _rte_get_timer_cycles(void) { return rte_get_timer_cycles(); }

uint64_t _rte_get_timer_hz(void) { return rte_get_timer_hz(); }

void _rte_delay_ms(unsigned ms) { rte_delay_ms(ms); }

uint64_t _rte_rdtsc(void) { return rte_rdtsc(); }

uint64_t _rte_rdtsc_precise(void) { return rte_rdtsc_precise(); }
...
char *_rte_mbuf_to_baddr(struct rte_mbuf *md) { return rte_mbuf_to_baddr(md); }

void *_rte_mbuf_to_priv(struct rte_mbuf *m) { return rte_mbuf_to_priv(m); }
...
void _rte_mbuf_raw_free(struct rte_mbuf *m) { rte_mbuf_raw_free(m); }
...
void _rte_pktmbuf_reset(struct rte_mbuf *m) { rte_pktmbuf_reset(m); }
...
void _rte_pktmbuf_attach_extbuf(struct rte_mbuf *m, void *buf_addr,
...
void _rte_pktmbuf_detach(struct rte_mbuf *m) { rte_pktmbuf_detach(m); }

struct rte_mbuf *_rte_pktmbuf_prefree_seg(struct rte_mbuf *m) {
  return rte_pktmbuf_prefree_seg(m);
}

void _rte_pktmbuf_free_seg(struct rte_mbuf *m) { rte_pktmbuf_free_seg(m); }

void _rte_pktmbuf_free(struct rte_mbuf *m) { rte_pktmbuf_free(m); }
...
// int
// _rte_eth_rx_descriptor_done(uint16_t port_id, uint16_t queue_id, uint16_t
// offset) {
...
int _rte_vlan_strip(struct rte_mbuf *m) { return rte_vlan_strip(m); }

int _rte_vlan_insert(struct rte_mbuf **m) { return rte_vlan_insert(m); }
