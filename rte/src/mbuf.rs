...
bitflags! {
    pub struct OffloadFlags: u64 {
        ...
        const RTE_MBUF_F_RX_VLAN_PKT      = ffi::PKT_RX_VLAN as u64;
        /// RX packet with RSS hash result.
        const RTE_MBUF_F_RX_RSS_HASH      = ffi::PKT_RX_RSS_HASH as u64;
        /// RX packet with FDIR match indicate.
        const RTE_MBUF_F_RX_FDIR          = ffi::PKT_RX_FDIR as u64;
        /// External IP header checksum error.
        // const RTE_MBUF_F_RX_EIP_CKSUM_BAD = ffi::PKT_RX_EIP_CKSUM_BAD as u64;
        ...
        const RTE_MBUF_F_RX_VLAN_STRIPPED = ffi::PKT_RX_VLAN_STRIPPED as u64;

        /// Mask of bits used to determine the status of RX IP checksum.
        const RTE_MBUF_F_RX_IP_CKSUM_MASK    = ffi::PKT_RX_IP_CKSUM_MASK as u64;
        /// no information about the RX IP checksum
        const RTE_MBUF_F_RX_IP_CKSUM_UNKNOWN = ffi::PKT_RX_IP_CKSUM_UNKNOWN as u64;
        /// the IP checksum in the packet is wrong
        const RTE_MBUF_F_RX_IP_CKSUM_BAD     = ffi::PKT_RX_IP_CKSUM_BAD as u64;
        /// the IP checksum in the packet is valid
        const RTE_MBUF_F_RX_IP_CKSUM_GOOD    = ffi::PKT_RX_IP_CKSUM_GOOD as u64;
        /// the IP checksum is not correct in the packet data,
        /// but the integrity of the IP header is verified.
        const RTE_MBUF_F_RX_IP_CKSUM_NONE    = ffi::PKT_RX_IP_CKSUM_NONE as u64;

        /// Mask of bits used to determine the status of RX IP checksum.
        const RTE_MBUF_F_RX_L4_CKSUM_MASK      = ffi::PKT_RX_L4_CKSUM_MASK as u64;
        /// no information about the RX L4 checksum
        const RTE_MBUF_F_RX_L4_CKSUM_UNKNOWN   = ffi::PKT_RX_L4_CKSUM_UNKNOWN as u64;
        /// the L4 checksum in the packet is wrong
        const RTE_MBUF_F_RX_L4_CKSUM_BAD       = ffi::PKT_RX_L4_CKSUM_BAD as u64;
        /// the L4 checksum in the packet is valid
        const RTE_MBUF_F_RX_L4_CKSUM_GOOD      = ffi::PKT_RX_L4_CKSUM_GOOD as u64;
        /// the L4 checksum is not correct in the packet data,
        /// but the integrity of the IP header is verified.
        const RTE_MBUF_F_RX_L4_CKSUM_NONE      = ffi::PKT_RX_L4_CKSUM_NONE as u64;

        /// RX IEEE1588 L2 Ethernet PT Packet.
        const RTE_MBUF_F_RX_IEEE1588_PTP  = ffi::PKT_RX_IEEE1588_PTP as u64;
        /// RX IEEE1588 L2/L4 timestamped packet.
        const RTE_MBUF_F_RX_IEEE1588_TMST = ffi::PKT_RX_IEEE1588_TMST as u64;
        /// FD id reported if FDIR match.
        const RTE_MBUF_F_RX_FDIR_ID       = ffi::PKT_RX_FDIR_ID as u64;
        /// Flexible bytes reported if FDIR match.
        const RTE_MBUF_F_RX_FDIR_FLX      = ffi::PKT_RX_FDIR_FLX as u64;
        /// RX packet with double VLAN stripped.
        const RTE_MBUF_F_RX_QINQ_STRIPPED = ffi::PKT_RX_QINQ_STRIPPED as u64;

        /// When packets are coalesced by a hardware or virtual driver, this flag
        /// can be set in the RX mbuf, meaning that the m->tso_segsz field is
        /// valid and is set to the segment size of original packets.
        const RTE_MBUF_F_RX_LRO = ffi::PKT_RX_LRO as u64;

        /// Indicate that security offload processing was applied on the RX packet.
        const RTE_MBUF_F_RX_SEC_OFFLOAD = ffi::PKT_RX_SEC_OFFLOAD as u64;

        /// Indicate that security offload processing failed on the RX packet.
        const RTE_MBUF_F_RX_SEC_OFFLOAD_FAILED = ffi::PKT_RX_SEC_OFFLOAD_FAILED as u64;

        /// The RX packet is a double VLAN, and the outer tci has been
        /// saved in in mbuf->vlan_tci_outer. If RTE_MBUF_F_RX_QINQ set, RTE_MBUF_F_RX_VLAN
        ...
        const RTE_MBUF_F_RX_QINQ = ffi::PKT_RX_QINQ as u64;

        /// Mask of bits used to determine the status of outer RX L4 checksum.
        const RTE_MBUF_F_RX_OUTER_L4_CKSUM_MASK	= ffi::PKT_RX_OUTER_L4_CKSUM_MASK as u64;
        /// no info about the outer RX L4 checksum
        const RTE_MBUF_F_RX_OUTER_L4_CKSUM_UNKNOWN = ffi::PKT_RX_OUTER_L4_CKSUM_UNKNOWN as u64;
        /// the outer L4 checksum in the packet is wrong
        const RTE_MBUF_F_RX_OUTER_L4_CKSUM_BAD	    = ffi::PKT_RX_OUTER_L4_CKSUM_BAD as u64;
        /// the outer L4 checksum in the packet is valid
        const RTE_MBUF_F_RX_OUTER_L4_CKSUM_GOOD    = ffi::PKT_RX_OUTER_L4_CKSUM_GOOD as u64;
        /// invalid outer L4 checksum state.
        const RTE_MBUF_F_RX_OUTER_L4_CKSUM_INVALID	= ffi::PKT_RX_OUTER_L4_CKSUM_INVALID as u64;

        /// Indicate that the metadata field in the mbuf is in use.
        // const RTE_MBUF_F_TX_METADATA = ffi::PKT_TX_METADATA as u64;
        ...
        const RTE_MBUF_F_TX_OUTER_UDP_CKSUM = ffi::PKT_TX_OUTER_UDP_CKSUM as u64;

         /// UDP Fragmentation Offload flag. This flag is used for enabling UDP
         /// fragmentation in SW or in HW. When use UFO, mbuf->tso_segsz is used
         /// to store the MSS of UDP fragments.
        const RTE_MBUF_F_TX_UDP_SEG = ffi::PKT_TX_UDP_SEG as u64;

        /// Request security offload processing on the TX packet.
        const RTE_MBUF_F_TX_SEC_OFFLOAD = ffi::PKT_TX_SEC_OFFLOAD as u64;

        /// Offload the MACsec. This flag must be set by the application to enable
        /// this offload feature for a packet to be transmitted.
        const RTE_MBUF_F_TX_MACSEC = ffi::PKT_TX_MACSEC as u64;
        ...
        const RTE_MBUF_F_TX_TUNNEL_VXLAN  = ffi::PKT_TX_TUNNEL_VXLAN as u64;
        const RTE_MBUF_F_TX_TUNNEL_GRE    = ffi::PKT_TX_TUNNEL_GRE as u64;
        const RTE_MBUF_F_TX_TUNNEL_IPIP   = ffi::PKT_TX_TUNNEL_IPIP as u64;
        const RTE_MBUF_F_TX_TUNNEL_GENEVE = ffi::PKT_TX_TUNNEL_GENEVE as u64;
        /// TX packet with MPLS-in-UDP RFC 7510 header.
        const RTE_MBUF_F_TX_TUNNEL_MPLSINUDP = ffi::PKT_TX_TUNNEL_MPLSINUDP as u64;
        const RTE_MBUF_F_TX_TUNNEL_VXLAN_GPE = ffi::PKT_TX_TUNNEL_VXLAN_GPE as u64;
        ...
        const RTE_MBUF_F_TX_TUNNEL_IP = ffi::PKT_TX_TUNNEL_IP as u64;
         ...
         /// Specific tunnel headers that contain payload length, sequence id
         /// or checksum are not expected to be updated.
        const RTE_MBUF_F_TX_TUNNEL_UDP = ffi::PKT_TX_TUNNEL_UDP as u64;

        const RTE_MBUF_F_TX_TUNNEL_MASK = ffi::PKT_TX_TUNNEL_MASK as u64;

        /// TX packet with double VLAN inserted.
        const RTE_MBUF_F_TX_QINQ = ffi::PKT_TX_QINQ as u64;

        /// TX packet with double VLAN inserted.
        // const RTE_MBUF_F_TX_QINQ_PKT = ffi::PKT_TX_QINQ_PKT as u64;
         ...
         ///  - if it's IPv4, set the RTE_MBUF_F_TX_IP_CKSUM flag and write the IP checksum
         ///    to 0 in the packet
         ///  - fill the mbuf offload information: l2_len, l3_len, l4_len, tso_segsz
         ///  - calculate the pseudo header checksum without taking ip_len in account,
         ///    and set it in the TCP header. Refer to rte_ipv4_phdr_cksum() and
         ///    rte_ipv6_phdr_cksum() that can be used as helpers.
        const RTE_MBUF_F_TX_TCP_SEG = ffi::PKT_TX_TCP_SEG as u64;

        /// TX IEEE1588 packet to timestamp.
        const RTE_MBUF_F_TX_IEEE1588_TMST = ffi::PKT_TX_IEEE1588_TMST as u64;
        ...
        const RTE_MBUF_F_TX_L4_NO_CKSUM   = ffi::PKT_TX_L4_NO_CKSUM as u64;
        /// TCP cksum of TX pkt. computed by NIC.
        const RTE_MBUF_F_TX_TCP_CKSUM     = ffi::PKT_TX_TCP_CKSUM;
        /// SCTP cksum of TX pkt. computed by NIC.
        const RTE_MBUF_F_TX_SCTP_CKSUM    = ffi::PKT_TX_SCTP_CKSUM;
        /// UDP cksum of TX pkt. computed by NIC.
        const RTE_MBUF_F_TX_UDP_CKSUM     = ffi::PKT_TX_UDP_CKSUM;
        /// Mask for L4 cksum offload request.
        const RTE_MBUF_F_TX_L4_MASK       = ffi::PKT_TX_L4_MASK;
        ...
        const RTE_MBUF_F_TX_IP_CKSUM = ffi::PKT_TX_IP_CKSUM;

         /// Packet is IPv4. This flag must be set when using any offload feature
         /// (TSO, L3 or L4 checksum) to tell the NIC that the packet is an IPv4
         /// packet. If the packet is a tunneled packet, this flag is related to
         /// the inner headers.
        const RTE_MBUF_F_TX_IPV4 = ffi::PKT_TX_IPV4;

         /// Packet is IPv6. This flag must be set when using an offload feature
         /// (TSO or L4 checksum) to tell the NIC that the packet is an IPv6
         /// packet. If the packet is a tunneled packet, this flag is related to
         /// the inner headers.
        const RTE_MBUF_F_TX_IPV6          = ffi::PKT_TX_IPV6;

        /// TX packet is a 802.1q VLAN packet.
        // const RTE_MBUF_F_TX_VLAN_PKT      = ffi::PKT_TX_VLAN_PKT;

         /// Offload the IP checksum of an external header in the hardware. The
         /// flag RTE_MBUF_F_TX_OUTER_IPV4 should also be set by the application, alto ugh
         /// a PMD will only check RTE_MBUF_F_TX_IP_CKSUM.  The IP checksum field in the
         /// packet must be set to 0.
         ///  - set the outer IP checksum field in the packet to 0
         ///  - fill the mbuf offload information: outer_l2_len, outer_l3_len
        const RTE_MBUF_F_TX_OUTER_IP_CKSUM   = ffi::PKT_TX_OUTER_IP_CKSUM;

         /// Packet outer header is IPv4. This flag must be set when using any
         /// outer offload feature (L3 or L4 checksum) to tell the NIC that the
         /// outer header of the tunneled packet is an IPv4 packet.
        const RTE_MBUF_F_TX_OUTER_IPV4   = ffi::PKT_TX_OUTER_IPV4;

         /// Packet outer header is IPv6. This flag must be set when using any
         /// outer offload feature (L4 checksum) to tell the NIC that the outer
         /// header of the tunneled packet is an IPv6 packet.
        const RTE_MBUF_F_TX_OUTER_IPV6    = ffi::PKT_TX_OUTER_IPV6;

        /// Bitmask of all supported packet Tx offload features flags,
        /// which can be set for packet.
        const RTE_MBUF_F_TX_OFFLOAD_MASK  = ffi::PKT_TX_OFFLOAD_MASK;
...
impl MBuf {
    ...
    pub fn prefetch_part1(&self) {
        unsafe { ffi::_rte_mbuf_prefetch_part1(self.as_raw_mut()) }
    }

    /// Prefetch the second part of the mbuf
    pub fn prefetch_part2(&self) {
        unsafe { ffi::_rte_mbuf_prefetch_part2(self.as_raw_mut()) }
    }

    /// Return the mbuf owning the data buffer address of an indirect mbuf.
    pub fn from_indirect(other: &MBuf) -> Self {
        unsafe { ffi::_rte_mbuf_from_indirect(other.as_raw_mut()) }.into()
    }

    /// Return the buffer address embedded in the given mbuf.
    pub fn buf_addr(&self) -> NonNull<u8> {
        NonNull::new(unsafe { ffi::_rte_mbuf_to_baddr(self.as_raw_mut()) })
            .unwrap()
            .cast()
    }
    ...
    pub fn priv_addr(&self) -> NonNull<u8> {
        NonNull::new(unsafe { ffi::_rte_mbuf_to_priv(self.as_raw_mut()) })
            .unwrap()
            .cast()
    }
    ...
    pub fn free_seg(&mut self) {
        unsafe { ffi::_rte_pktmbuf_free_seg(self.as_raw_mut()) }
    }
    ...
    pub fn free(&mut self) {
        unsafe { ffi::_rte_pktmbuf_free(self.as_raw_mut()) }
    }
    ...
    pub fn raw_free(&mut self) {
        ...
        unsafe { ffi::_rte_mbuf_raw_free(self.as_raw_mut()) }
    }
    ...
    pub fn refcnt_set(&mut self, new: u16) {
        unsafe { ffi::_rte_mbuf_refcnt_set(self.as_raw_mut(), new) }
    }

    /// Adds given value to an mbuf's refcnt and returns its new value.
    pub fn refcnt_update(&mut self, new: i16) -> u16 {
        unsafe { ffi::_rte_mbuf_refcnt_update(self.as_raw_mut(), new) }
    }
    ...
    pub fn reset_headroom(&mut self) {
        unsafe { ffi::_rte_pktmbuf_reset_headroom(self.as_raw_mut()) }
    }

    /// Reset the fields of a packet mbuf to their default values.
    pub fn reset(&mut self) {
        unsafe { ffi::_rte_pktmbuf_reset(self.as_raw_mut()) }
    }
    ...
    pub fn lastseg(&self) -> Self {
        unsafe { ffi::_rte_pktmbuf_lastseg(self.as_raw_mut()) }.into()
    }
    ...
    pub fn data_iova(&self) -> ffi::rte_iova_t {
        self.buf_iova + self.data_off as u64
    }
    ...
    pub fn prepend(&mut self, len: usize) -> Result<NonNull<u8>> {
        unsafe { ffi::_rte_pktmbuf_prepend(self.as_raw_mut(), len as u16) }
            .as_result()
            .map(|p| p.cast())
    }
    ...
    pub fn append(&mut self, len: usize) -> Result<NonNull<u8>> {
        unsafe { ffi::_rte_pktmbuf_append(self.as_raw_mut(), len as u16) }
            .as_result()
            .map(|p| p.cast())
    }
    ...
    pub fn adj(&mut self, len: usize) -> Result<NonNull<u8>> {
        unsafe { ffi::_rte_pktmbuf_adj(self.as_raw_mut(), len as u16) }
            .as_result()
            .map(|p| p.cast())
    }
    ...
    pub fn trim(&mut self, len: usize) -> Result<()> {
        unsafe { ffi::_rte_pktmbuf_trim(self.as_raw_mut(), len as u16) }
            .as_result()
            .map(|_| ())
    }
    ...
    pub fn chain(&self, tail: &Self) -> Result<()> {
        unsafe { ffi::_rte_pktmbuf_chain(self.as_raw_mut(), tail.as_raw_mut()) }
            .as_result()
            .map(|_| ())
    }
    ...
    pub fn linearize(&self) -> Result<()> {
        unsafe { ffi::_rte_pktmbuf_linearize(self.as_raw_mut()) }
            .as_result()
            .map(|_| ())
    }
    ...
    pub fn dump<S: AsRawFd>(&self, s: &S, dump_len: usize) {
        if let Ok(f) = cfile::fdopen(s, "w") {
            unsafe {
                ffi::rte_pktmbuf_dump(f.as_ptr() as *mut ffi::FILE, self.as_raw(), dump_len as u32);
            }
        }
    }
}
...
impl ExtSharedInfo {
    ...
    pub fn refcnt_set(&mut self, new: u16) {
        unsafe { ffi::_rte_mbuf_ext_refcnt_set(self.as_raw_mut(), new) }
    }

    /// Add given value to refcnt of an external buffer and return its new value.
    pub fn refcnt_update(&mut self, new: i16) -> u16 {
        unsafe { ffi::_rte_mbuf_ext_refcnt_update(self.as_raw_mut(), new) }
    }
}
...
impl MBuf {
    ...
    pub fn attach(&mut self, m: &MBuf) {
        unsafe { ffi::_rte_pktmbuf_attach(self.as_raw_mut(), m.as_raw_mut()) }
    }

    /// Detach a packet mbuf from external buffer or direct buffer.
    pub fn detach(&mut self) {
        unsafe { ffi::_rte_pktmbuf_detach(self.as_raw_mut()) }
    }
}
...
impl MBufPool for mempool::MemoryPool {
    fn data_room_size(&self) -> usize {
        unsafe { ffi::_rte_pktmbuf_data_room_size(self.as_raw_mut()) as usize }
    }

    fn priv_size(&self) -> usize {
        unsafe { ffi::_rte_pktmbuf_priv_size(self.as_raw_mut()) as usize }
    }

    fn alloc(&mut self) -> Result<MBuf> {
        unsafe { ffi::_rte_pktmbuf_alloc(self.as_raw_mut()) }
            .as_result()
            .map(MBuf)
    }

    fn alloc_bulk(&mut self, mbufs: &mut [Option<MBuf>]) -> Result<()> {
        unsafe { ffi::_rte_pktmbuf_alloc_bulk(self.as_raw_mut(), mbufs.as_mut_ptr() as *mut _, mbufs.len() as u32) }
            .as_result()
            .map(|_| ())
    }

    fn clone(&mut self, mbuf: &MBuf) -> Result<MBuf> {
        unsafe { ffi::_rte_pktmbuf_clone(mbuf.as_raw_mut(), self.as_raw_mut()) }
            .as_result()
            .map(MBuf)
    }
}
